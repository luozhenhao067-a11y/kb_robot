...
class node_content_split(NodeBase):
    ...
    def process(self,state: ImportGraphState):
        # 防御性编程
        file_title, md_content, md_path_obj = self.check_file(state)
        # 粗切
        sec_list = self.rough_cut(file_title, md_content)
        # 接下来递归细切
        final_sec_list = self.spliter_cut(file_title, md_path_obj, sec_list)
        return {
            'chunks': final_sec_list
        }

    def spliter_cut(self, file_title: str, md_path_obj: Path, sec_list: list[Any]) -> list[Any]:
        chunk_size = 200
        chunk_overlap = 20
